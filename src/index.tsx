...
// Runtime seed – fires on login if admin row missing (extra safety net)
...
const authMiddleware = async (c: any, next: any) => {
  ...
  try {
    const payload = await verifyToken(token, c.env.JWT_SECRET || 'aes-default-secret-change-me')
...
async function updateJobStatus(db: D1Database, jobId: string) {
  ...
  else if (!anyUnderRepair) newStatus = 'repaired'
...
app.post('/api/auth/login', async (c) => {
  try { await seedAdmin(c.env.DB) } catch (_) { /* schema may not exist yet on cold start */ }
  let body: any
  ...
  const token = await signToken(
    ...
    c.env.JWT_SECRET || 'aes-default-secret-change-me'
  )
...
app.get('/api/jobs', authMiddleware, async (c) => {
  ...
  if (search) {
    conditions.push(`(j.snap_name LIKE ? OR j.snap_mobile LIKE ? OR j.id LIKE ?)`)
  ...
  // Add balance_due to each (safe for staff too — total_charges hidden in frontend)
  ...
  await c.env.DB.prepare(
    `INSERT INTO jobs(id,customer_id,snap_name,snap_mobile,snap_mobile2,
     ...
     VALUES(?,?,?,?,?,?,?,?)`
         ...
         body.note || null, body.received_amount || 0).run()
...
app.get('/api/jobs/:id', authMiddleware, async (c) => {
  ...
  const enriched = machines.map((m: any) => ({
    ...m,
    images: (() => {
      try { return JSON.parse(m.images_json || '[]') } catch { return [] }
    })()
  }))
...
app.put('/api/jobs/:id', authMiddleware, async (c) => {
  ...
  if (body.status === 'delivered') {
    if (!body.delivered_at) fields.push(`delivered_at=datetime('now')`)
  }
...
app.delete('/api/jobs/:id', authMiddleware, adminOnly, async (c) => {
  ...
  const { results: imgs } = await c.env.DB.prepare(
    `SELECT mi.r2_object_key
     FROM machine_images mi
     JOIN machines m ON mi.machine_id=m.id
     WHERE m.job_id=?`
  ).bind(id).all<any>()
  for (const img of imgs) {
    if (img.r2_object_key) {
      try { await c.env.PRODUCT_IMAGES.delete(img.r2_object_key) } catch (_) {}
    }
  }
  // CASCADE handles machine_images → machines automatically
...
app.delete('/api/machines/:id', authMiddleware, adminOnly, async (c) => {
  ...
  // CASCADE removes machine_images automatically; explicit delete is belt-and-suspenders
...
app.delete('/api/images/:imageId', authMiddleware, adminOnly, async (c) => {
  ...
  const img = await c.env.DB.prepare(
    'SELECT * FROM machine_images WHERE id=?'
  ).bind(imageId).first<any>()
...
app.post('/api/backup/import', authMiddleware, adminOnly, async (c) => {
  ...
  for (const r of machines) {
    await c.env.DB.prepare(
      `INSERT INTO machines(id,job_id,product_name,product_complaint,charges,quantity,
       ...
       VALUES(?,?,?,?,?,?,?,?,?,?)
       ON CONFLICT(id) DO UPDATE SET
         status=excluded.status, charges=excluded.charges`
  ...
  // Advance counter to avoid collision
...
app.delete('/api/cleanup', authMiddleware, adminOnly, async (c) => {
  ...
  if (full_reset) {
    // Delete leaf tables first to respect FKs (CASCADE should handle sub-tables)
